...
## BT-D28 — Replace eSpeak local testing/fallback with Pocket TTS — 2026-09-16

The task owner approved replacing the previously validated eSpeak local provider after controlled diagnostics showed that a perceived response "loop" was not an STT/self-audio loop: each of five caller utterances mapped to one AI turn and no additional StartOfTurn/EndOfTurn occurred during the long playback. A direct eSpeak synthesis also completed normally, while the owner found the voice too generic/unclear for reliable functional testing.

Before changing repository code, Pocket TTS 3.1.0 and Supertonic 3 were compared on the reference MSI laptop. Pocket native streaming produced roughly 101–105 ms first audio in the three-sentence comparison and then 20/20 successful warm generations with zero failures, 75.1–88.8 ms TTFA and 78.3 ms average TTFA. Supertonic short-phrase synthesis completion was roughly 467–490 ms. An 8 kHz G.711 mu-law blind listening pass preferred Pocket in two of three pairs. These are reference-machine local measurements, not caller-heard latency evidence.
